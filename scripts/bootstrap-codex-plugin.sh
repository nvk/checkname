...
plugin-enable block for @checkname.
...
if [[ -z "$MARKETPLACE_SOURCE" ]]; then
  ...
  if [[ "$MARKETPLACE_SOURCE" != "$ROOT" ]]; then
    ...
    echo "This helper will not overwrite another source automatically." >&2
    echo "Use a clean Codex home, or remove/re-add the marketplace first." >&2
    exit 1
  fi
fi
...
block = sys.argv[2]
begin = "# BEGIN checkname Codex bootstrap"
end = "# END checkname Codex bootstrap"
managed = f"{begin}\n{block}\n{end}\n"
