...
PROJECT_ROOT="${PWD}"
USER_HOME="${HOME}"
MARKETPLACE_NAME="checkname"
PLUGIN_KEY="checkname@${MARKETPLACE_NAME}"

usage() {
  cat <<'EOF'
Usage: ./scripts/verify-codex-plugin.sh [options]

Verify that Codex resolves @checkname to this repo's generated Codex skill.
...
EXPECTED_SKILL_PATH="$ROOT/plugins/checkname/skills/checkname/SKILL.md"
TMP_OUTPUT="$(mktemp)"
...
if [[ "$SCOPE" == "project" ]]; then
  HOME="$USER_HOME" codex -C "$PROJECT_ROOT" debug prompt-input '@checkname northstar' >"$TMP_OUTPUT"
else
  HOME="$USER_HOME" codex -C "$PROBE_DIR" debug prompt-input '@checkname northstar' >"$TMP_OUTPUT"
fi

if grep -Fq 'checkname:checkname' "$TMP_OUTPUT" && grep -Fq "$EXPECTED_SKILL_PATH" "$TMP_OUTPUT"; then
  echo "OK: Codex resolves @checkname from this repo."
  echo "Skill path:"
...
match = re.search(r'(/[^\n"]*skills/checkname/SKILL\.md)', text)
print(match.group(1) if match else "")
PY
)"

if [[ -n "$ACTUAL_SKILL_PATH" ]]; then
  echo "FAIL: Codex resolved @checkname, but not from this repo." >&2
  echo "Resolved skill path:" >&2
  echo "  $ACTUAL_SKILL_PATH" >&2
  echo "Expected skill path:" >&2
  echo "  $EXPECTED_SKILL_PATH" >&2
  echo "This usually means another Codex home or cached checkout already owns the 'checkname' marketplace." >&2
  exit 1
fi

echo "PENDING: Codex did not expose @checkname in this headless session." >&2
echo "The marketplace and config are present, but Codex may still require the interactive /plugins UI to materialize or enable the plugin on first install." >&2
echo >&2
echo "Next step:" >&2
echo "  1. Start Codex with HOME set to this Codex home (if non-default)." >&2
echo "  2. Open /plugins and enable 'checkname'." >&2
